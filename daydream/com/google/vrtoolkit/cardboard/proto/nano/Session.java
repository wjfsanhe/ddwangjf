...
{
    public static final class TrackerState extends ExtendableMessageNano
        implements Cloneable
    {
        ...
        public final TrackerState setTimeSinceEpochSeconds(long l)
        {
        ...
        public final TrackerState clone()
        {
            ...
            try
            {
                trackerstate = (TrackerState)super.clone();
            }
            ...
            if(q != null && q.length > 0)
                trackerstate.q = (double[])q.clone();
            if(gyroscopeBias != null && gyroscopeBias.length > 0)
                trackerstate.gyroscopeBias = (double[])gyroscopeBias.clone();
            if(lensOffset != null && lensOffset.length > 0)
                trackerstate.lensOffset = (float[])lensOffset.clone();
            if(lastGyroscopeSample != null && lastGyroscopeSample.length > 0)
                trackerstate.lastGyroscopeSample = (double[])lastGyroscopeSample.clone();
            return trackerstate;
        }
        ...
        public static TrackerState parseFrom(byte abyte0[])
            throws InvalidProtocolBufferNanoException
        {
            return (TrackerState)MessageNano.mergeFrom(new TrackerState(), abyte0);
        }
